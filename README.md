...
The following table shows the impact of adding each of the following features had on the accuracy, precision, and recall of the final VotingClassifier used.  In most cases, performance in accuracy, precision, and recall decreased or remained the same.

<table>
<tr>
<td><b><u>Feature added</u></b>
</td>
<td><b><u>Accuracy</u></b>
</td>
...
The features expenses, director_fees, loan_advances, and restricted_stock_deferred all showed similar recall performance to the VotingClassifier described below.  These features were added to the classifier and the results were an accuracy of 0.87, a precision of 0.60, and a recall of 0.37.  A slight decrease in precision was seen and a slight increase in recall was seen.  The features were left off the model to include fewer features to avoid overfitting.

Two features were created from the original features: ratios of bonus to salary and from_messages to to_messages.  The thought was that perhaps POIs tended to have large bonuses compared to their salaries.  Another thought was that the ratio for from_messages to to_messages would be smaller for POIs since they may receive more emails than they send.  The two ratios created did not have a discernible impact on algorithm performance and were not included in the final features used.  The results of adding these to the classifier are summarized below.  In each case, the accuracy, precision, and recall went down when these features were added (as compared to the VotingClassifier described below).
